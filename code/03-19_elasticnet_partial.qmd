...
my note: beta are the coefficient
...
## Lasso penalty (L1)  
The lasso (least absolute shrinkage and selection operator) penalty is an alternative to the ridge penalty that requires only a small modification. The only difference is that we swap out the L2 norm for an L1 norm.  

my note: This adds the sum of absolute coefficient values.
...
mynote: This adds the sum of absolute coefficient values.
...
```
My note: Each time you you split the data usibng the code below and run the a density plot (the one above), it changes because it is randomly selecting observations in the training and testing data.
...
my note: then added strata = strength_gtex
weather_split <- initial_split(weather,
                               prop = 0.7,
                               strata = strength_gtex)
...
weather_recipe <-
  ...
  recipe(strength_gtex ~ .,
         data = weather_train)%>% #. means selecting all the check video 
  ...
  # Normalizing: this isn't done in this exercise but at the end you can run the codes below to see the difference on the final outcome.
  #step_normalize(all_numeric_predictors()) %>%
  

  # Decorrelating: this isn't done in this exercise but at the end you can run the codes below to see the difference on the final outcome.
  #step_pca(all_numeric_predictors(), threshold = .8) #threshold states how many PCAs it should keep
...
RMSE is smaller on the training set than on the test set. While, the rsq is higher on the training set than on the test data.

Why?
This could be because the model is trained on the train set as opposed to the test set.
